...
async function sendCaregiverAlert({ client, user, prevIntervalEnd, timezone, nowUTC, actions }) {
  const { user_id, user_name, user_email, caregiver_name, caregiver_email, interval } = user;
...
async function sendUserReminder({ client, user, intervalStart, intervalEnd, timezone, nowUTC, actions }) {
  const { user_id, user_name, user_email, caregiver_email, interval } = user;
  await sendEmail({
    to: user_email,
    ...
    for (const user of usersResult.rows) {
      ...
      if (now >= prevIntervalEnd && now < prevIntervalEnd.plus({ hours: 1 })) {
        ...
        if (checkinResult.rows.length === 0) {
          await sendCaregiverAlert({ client, user, prevIntervalEnd, timezone, nowUTC, actions });
          alertsSent++;
        }
      }
      ...
      if (intervalEnd > now && intervalEnd <= now.plus({ hours: 1 })) {
        ...
        if (checkinResult.rows.length === 0) {
          await sendUserReminder({ client, user, intervalStart, intervalEnd, timezone, nowUTC, actions });
          remindersSent++;
        }
      }
    }
